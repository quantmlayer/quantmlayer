...
/// Every endpoint in a subtree, so the footer can describe what is on screen.
fn collect_endpoints(node: &TreeNode) -> Vec<&Endpoint> {
    let mut out: Vec<&Endpoint> = node.endpoints.iter().collect();
    for k in &node.children {
        out.extend(collect_endpoints(k));
    }
    out
}
...
pub fn render(tree: &Tree, source: &str, head: &str, records: usize) -> String {
    ...
    // Explain only what this page actually shows. Observe-mode endpoints carry
    // `proto ip:port` from the syscall tracer; enforce-mode ones carry the
    // hostname the broker was asked for. Printing the tracer's caveats on a
    // page with no tracer output describes a feature that is not on screen —
    // which is exactly the kind of small mismatch a careful reader pulls on.
    let endpoints: Vec<&Endpoint> = tree.roots.iter().flat_map(collect_endpoints).collect();
    let has_proto = endpoints
        .iter()
        .any(|e| e.endpoint.starts_with("udp ") || e.endpoint.starts_with("tcp "));
    let notes = if has_proto {
        "Endpoints show the socket protocol where the <code>socket()</code> call was observed: \
         <code>udp ...:0</code> is a source-address lookup that sends nothing, \
         <code>tcp ...:443</code> is a session. Most connections carry no outcome - a \
         non-blocking <code>connect</code> returns <code>EINPROGRESS</code> and its result \
         arrives later through <code>SO_ERROR</code>, which a syscall tracer never sees. "
    } else {
        "Endpoints are the destinations the broker was asked to reach, as hostnames. "
    };
...
<footer>
{notes}Generated by QuantmLayer; opens offline, loads nothing.
</footer>
</body></html>
"#,
        notes = notes,
...
mod tests {
    ...
    /// The footer describes what the page shows. Enforce-mode endpoints are
    /// hostnames from the broker, with no protocol and no syscall-level
    /// outcome — explaining `udp ...:0` and `EINPROGRESS` there describes a
    /// feature that is not on screen.
    #[test]
    fn the_footer_matches_the_endpoints_shown() {
        let n = node(1, "curl", "/usr/bin/curl");
        let mut c = conn(1, "pastebin.com:443", 2);
        c.comm = Some("curl".into());
        let broker = render(&build_with_connects(&[n], &[c], 0), "r.jsonl", "a", 1);
        assert!(broker.contains("destinations the broker"), "wrong footer");
        assert!(
            !broker.contains("EINPROGRESS"),
            "tracer caveats on a broker page"
        );

        let n2 = node(2, "curl", "/usr/bin/curl");
        let mut c2 = conn(2, "udp 8.8.8.8:53", 2);
        c2.comm = Some("curl".into());
        let traced = render(&build_with_connects(&[n2], &[c2], 0), "r.jsonl", "a", 1);
        assert!(traced.contains("EINPROGRESS"), "missing tracer caveats");
        assert!(!traced.contains("destinations the broker"), "wrong footer");
    }
