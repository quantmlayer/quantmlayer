...
fn print_usage() {
    eprintln!(
        "ql {VERSION} — security runtime for coding agents\n\
         ...
         \x20 ql run      --profile <p.yaml> | --agent <name> | --mcp [--observe [--strict]] [--workspace <dir>] [--audit <log.jsonl>] [--proposed <p.yaml>] [--issue-token <out.json>] [--system-id <id> [--model-version <v>]] [--require-signed] [--trust-signer <pubkey>]... [--expect-commit <hash>] [--expect-image <digest>] [--result-json <out.json>] [--verdicts <out.jsonl>] [--prune-provider] [--ci] [--phase <name>] [--verbose] [--broker] -- <cmd...>\n\
